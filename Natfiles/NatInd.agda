...
accumprop : {A : ℕ → Set} → (n : ℕ) → Accum A n → (l : ℕ) → l ≤ n → A l
accumprop Zero p Zero _ = p
accumprop Zero p (Suc n) ()
accumprop {A} (Suc n) p l _ with (tricho< l n)
accumprop {A} (Suc n) p l _ | Inl q = accumprop n (pr1× p) l (<to≤ q) 
accumprop {A} (Suc n) p l _ | Inr (Inl q) = accumprop n (pr1× p) l (==ℕto≤ {l} {n} q) 
accumprop {A} (Suc n) p l r | Inr (Inr q) = transp A (≤∧≥to==ℕ  {Suc n} {l} < q , r >) (pr2× p)
