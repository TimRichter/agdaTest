...
_+_ {n} (FinC m proof[m<n]) (FinC l proof[l<n]) | No ¬proof[m+ℕl]<n = FinC ((m +ℕ l) - n) proof[[m+ℕl]-n<n] where
  proof[[m+ℕl]-n<n] = trans≤ {Suc ((m +ℕ l) - n)} {(m +ℕ (Suc l)) - n} {n} proof[Suc[[m+ℕl]-n]≤[m+ℕSuc[l]]-n] proof[[m+ℕSuc[l]]-n≤n] where
    proof[Suc[[m+ℕl]-n]≤[m+ℕSuc[l]]-n] = ==ℕto≤ {Suc ((m +ℕ l) - n)} {(m +ℕ (Suc l)) - n} 
        (==to==ℕ (==sym {ℕ} {(m +ℕ (Suc l)) - n} {Suc ((m +ℕ l) - n)} ([Sucn]-misSuc[n-m]for[n≥m] (m +ℕ l) n proof[m+ℕl≥n]))) where
      proof[m+ℕl≥n] = fcases (tricho< (m +ℕ l) n) where
        fcases : (((m +ℕ l) < n) == True) +Set ((((m +ℕ l) ==ℕ n) == True) +Set ((n < (m +ℕ l)) == True)) → ((m +ℕ l) ≥ n) == True
